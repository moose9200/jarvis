...
- Backend test coverage push (T1-11 batch A+C+D): 34 new tests across `test_users_auth.py` (register/login/JWT round-trip/set_industry/oauth-code mint — 13 tests, routers/users.py at 90% coverage), `test_files.py` (upload/list/get/delete + size-cap + 502 paths — 9 tests, routers/files.py at 97% coverage), and `test_tools_dispatch.py` (send_email fallback chain, get_product_releases cross-user isolation, get_recent_mentions, connector fan-out, unknown-tool guard — 12 tests). Total backend coverage now **57%** (was ~25%); full suite 79 passed.
